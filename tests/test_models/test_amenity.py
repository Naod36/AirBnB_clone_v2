...
def test_to_dict_creates_dict(self):
    ...
    self.assertNotIn("_sa_instance_state", new_d)  # Use assertNotIn for negative check
    for attr in am.__dict__.keys():  # Use keys() to iterate over keys
        if attr != "_sa_instance_state":  # Use != for comparison instead of 'is not'
            self.assertIn(attr, new_d)  # Use assertIn for positive check
    self.assertIn("__class__", new_d)  # Check if '__class__' key exists in the dictionary
